...
def getImages(request):
    try:
        ...
        for cred in creds:
            try:
                print "Try to Create Keystone Client using un:%s pw:%s ten:%s auth_url: %s:%s/v1.0"%(cred.un,cred.pw,cred.tenent,cred.site.url,cred.site.authport)
                _keystone_ = ksclient.Client(username=cred.un,password=cred.pw,tenant_name=cred.tenent,auth_url="%s:%s/v1.0"%(cred.site.url,cred.site.authport))
                print "Success"
